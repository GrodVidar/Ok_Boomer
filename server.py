                    ...
                    for player_id, stats in game_state.players.items():
                        ...
                        if horizontal_ex:
                            ...
                            if horizontal_hit and vertical_hit and time.time() - stats['damaged'] > 3:
                                ...
                                # return damage_player(stats, player_id)
                        if stats['hp'] <= 0:
                            deaths.append(stats['name'])
                    if len(deaths) == len(game_state.players)-1:
                        for player_id, stats in game_state.players.items():
                            if not stats['name'] in deaths:
                                return {'winner': {player_id: stats['name']}}
                else:
...
def on_bomb(player_id, position, game_state, **kwargs):
    global game_on
...
if __name__ == "__main__":
    initial_game_state = GameState(players={}, bombs={}, explosions={}, winner={})
